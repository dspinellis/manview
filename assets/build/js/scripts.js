...
ManView.Behaviors.PageView = Essential.Behavior.extend({
  ...
  init: function() {
	...
	.then(dispatchSource)
	.catch(function() {
	  dispatchSource(this.computeTextSha256Sync(request.response));
	}.bind(this));
    }.bind(this);
    request.send();
  },
  ...
  computeTextSha256Sync: function(text) {
    var bytes = this.utf8Bytes(text);
  ...
  sha256: function(bytes) {
	...
	w[j] = ((bytes[i + j * 4] << 24) | (bytes[i + j * 4 + 1] << 16) |
      ...
      for (j = 16; j < 64; j++) {
	s0 = (this.rotateRight(w[j - 15], 7) ^ this.rotateRight(w[j - 15], 18) ^ (w[j - 15] >>> 3)) >>> 0;
	s1 = (this.rotateRight(w[j - 2], 17) ^ this.rotateRight(w[j - 2], 19) ^ (w[j - 2] >>> 10)) >>> 0;
	w[j] = (w[j - 16] + s0 + w[j - 7] + s1) >>> 0;
      ...
      for (j = 0; j < 64; j++) {
	s1 = (this.rotateRight(e, 6) ^ this.rotateRight(e, 11) ^ this.rotateRight(e, 25)) >>> 0;
	ch = ((e & f) ^ ((~e) & g)) >>> 0;
	temp1 = (hh + s1 + ch + k[j] + w[j]) >>> 0;
	s0 = (this.rotateRight(a, 2) ^ this.rotateRight(a, 13) ^ this.rotateRight(a, 22)) >>> 0;
	maj = ((a & b) ^ (a & c) ^ (b & c)) >>> 0;
	temp2 = (s0 + maj) >>> 0;
  ...
  rotateRight: function(value, bits) {
